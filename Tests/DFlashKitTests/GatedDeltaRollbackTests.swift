...
final class GatedDeltaRollbackTests: XCTestCase {
    ...
    /// `full_attention_interval = 2`, so layers 0 and 2 are gated-DeltaNet and
    /// layers 1 and 3 are full attention - the same mix as the real target, at a
    /// size where a forward is microseconds.
    ///
    /// `linear_key_head_dim` is 32 so the fused gated-delta kernel is exercised;
    /// it truncates key dimensions that are not a multiple of 32 and would
    /// otherwise route this test to the ops fallback.
    ...
    private func makeModel() throws -> Qwen35TextModel {
        let configuration = try JSONDecoder().decode(
            Qwen35TextConfiguration.self, from: Data(Self.configurationJSON.utf8))
